...
const ProfilePage = ({ user }: Props) => (
  <section className="flex items-center justify-center flex-col absolute left-0 bg-white p-6 !pt-2 md:p-12 lg:p-20 max-w-10xl w-full mx-auto paddings">
    <section className="flex items-center justify-between max-lg:flex-col gap-10 w-full">
      <div className="flex -mt-16 items-start flex-col w-full">
        <Image
          src={user?.avatarUrl}
          width={100}
          height={100}
      ...
      {user?.projects?.edges?.length > 0 ? (
        <Image
          ...
          width={739}
          height={554}
          className="rounded-xl mt-20 object-contain"
        />
      ) : (
        <Image
